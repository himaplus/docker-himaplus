...
echo "Checking directories: /root/himaplus-authn" 2>&1 | tee -a /root/log/execute.log
find /root/himaplus-authn -maxdepth 0 -exec ls -la {} + 2>&1 | tee -a /root/log/execute.log # ls -la /root/himaplus-authn/

# volumesされているはずのディレクトリが空(初回ビルド時)なら、
if [ -d "/root/himaplus-authn/" ] && [ -z "$(ls -A "/root/himaplus-authn/")" ]; then # -dでディレクトリかどうか確認し、かつ、-zでls -Aによるリスト化したフォルダ内のリストが空なら真
    # log
    echo Clone now... 2>&1 | tee -a /root/log/execute.log

    # ソースコードのリポジトリをクローン、
    git clone https://github.com/unSerori/himaplus-authn -b main /root/himaplus-authn/./
    # copyされたファイルを適切な場所に移行
    mv .env /root/himaplus-authn/
else
    # log
